...
//        testIllegal();
    ...
    private void update(){
        ...
        if (getKing(true) == null || getKing(false) == null) {
            gameOver = true; // End game or trigger a fix
    ...
    public void paintComponent(Graphics g){
        ...
        if (promotion){
            g2.drawString("Promote to: " , 810, 150);
            for (Piece piece : promoPieces)
                g2.drawImage(piece.image, piece.getX(piece.col), piece.getY(piece.row),
                        Board.SQUARE_SIZE, Board.SQUARE_SIZE, null);
        }else {
            if (currentColor == WHITE){
                g2.drawString("White's turn", 810,490);
                if (checkingP != null && checkingP.color == BLACK){
                    g2.setColor(Color.red);
                    g2.drawString("The King", 810, 530);
                    g2.drawString("is in check!", 810, 560);
                    }
            }else {
                g2.drawString("Black's turn", 810,120);
                if (checkingP != null && checkingP.color == WHITE) {
                    g2.setColor(Color.red);
                    g2.drawString("The King", 810, 50);
                    g2.drawString("is in check!", 810, 80);
                }
            }
        }

        if (gameOver) {
            // Determine the correct winner based on who is in check at the game over
            String winner = (currentColor == WHITE) ? "White Wins" : "Black Wins";  // Assumes currentColor is the loser
    ...
    private boolean isKingInCheck() {
        ...
        if (king == null) {
            return false; // No king to check for check condition
        }
    ...
    private Piece getKing(boolean opponent) {
        int targetColor = opponent ? (currentColor == WHITE ? BLACK : WHITE) : currentColor;
        for (Piece piece : simPieces) {
            if (piece.type == Type.KING && piece.color == targetColor) {
                return piece;
            }
        }
        return null; // Return null if no king found, should handle this return value
    }
